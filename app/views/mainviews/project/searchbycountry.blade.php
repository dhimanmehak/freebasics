...
<section>
    <div class="search-page-section">
        <div class="container">
            <h3 class="h3-tags">   
                <span style='margin-right:15px;'>{{trans('messages.showme')}} {{trans('messages.all')}} {{trans('messages.projects')}} {{trans('messages.in')}}</span>
                <span > 
                    <select style="overflow: auto;width: 270px; padding: 5px;height: 35px;" onChange="window.location.href=this.value">                           
                        @foreach($countries as $country)
                        <option value="{{URL::to('project/country')}}/{{$country->countryname}}" @if($country->countryname==$selected) selected="selected" @endif>{{$country->countryname}}</option>
                        @endforeach
                    </select>
