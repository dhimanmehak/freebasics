...
class BackingRepository implements IBackingRepository {
    ...
    public function getbackingbyuserid($id) {
        $backing = new Backing;
        return $backing->join('projects', 'projects.id', '=', 'backings.projectid')->join('users', 'users.id', '=', 'backings.userid')->where('backings.userid', '=', $id)->get(array('projects.id', 'projects.title', 'projects.projectimage', 'users.lastname', 'users.firstname'));
    }
