...
//        $data->dob = $input['dob'];
    ...
    public function updatelastlogin($email, $ipaddress) {
     ...
     public function updatelastlogintwitter($twitterid, $ipaddress) {
        ...
        $user = new User;
        $user_id = $user->where('twitterid', '=', $twitterid)
...
public function createinstagramaccount(array $input) {
    ...
    //create pledege count
public function createpledgecount(array $countinput) {
       
     
  
        $pledge_cat = $countinput['catid'];
        $pledge_user = $countinput['userid'];
		$pledge_type = $countinput['logintype'];


        $data = DB::table('pledge')->where('pledge_cat',$pledge_cat)->where('pledge_user',$pledge_user)->count();
        if($data == 0)
        {
        
    
        $id = DB::table('pledge')->insertGetId(
    array('pledge_cat'=> $pledge_cat,'pledge_user' => $pledge_user, 'pledge_type' => $pledge_type)
);
 return $id;
        }
      //  DB::insert('insert into pledge (pledge_cat, pledge_user,pledge_type) values ($pledge_cat,$pledge_user,$pledge_cat)');
    }
    //pledgecount end
